...
// Find difference between largest and smallest item in a sequence
let diffOfMaxMin values = Seq.max values - Seq.min values
...
1311	64	77	1328	1344	1248	1522	51	978	1535	1142	390	81	409	68	352"
    ...
    let inputParsed = 
        ...
        |> Seq.map (fun strArr -> Seq.map int strArr)

    let part1 =
        inputParsed
        |> Seq.sumBy diffOfMaxMin

    let part2 =
        seq {
            for row in inputParsed do
            for x in row do
            for y in row do
            yield (if x <> y && x % y = 0 then x / y else 0)
        }
        |> Seq.sum
